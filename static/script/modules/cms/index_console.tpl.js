...
function new_creat() {
    ...
    $("input[name='ptype']").attr("value", "console").val();
...
function getFiles(obj) {
    ...
    if (navigator.userAgent.indexOf("MSIE") != -1 && !obj.files) {
        ...
        if (pt == 'android' && !match_an.test(filePath)) {
            ...
            document.getElementById('light').style.display = 'none';
            document.getElementById('fade').style.display = 'none';
        } else if (pt == 'console' && !match_console.test(filePath)) {
            notice("<%'上传文件应以【.exe】格式结尾'|L%>");
            document.getElementById('light').style.display = 'none';
            document.getElementById('fade').style.display = 'none';
        }
        ...
        if (file.size >= 62914560 || file.size == 0) {
        ...
        if (pt == 'android' && !match_an.test(file.name)) {
            ...
            document.getElementById('light').style.display = 'none';
            document.getElementById('fade').style.display = 'none';
        } else if (pt == 'console' && !match_console.test(file.name)) {
        	alert(match_console.test(file.name));
            notice("<%'上传文件应以【.exe】格式结尾'|L%>");
            document.getElementById('light').style.display = 'none';
            document.getElementById('fade').style.display = 'none';
        }
...
function edit_console_dir() {
    if (pid == '' || pid == null || ptype != 'console') {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (flag == 'edit') {
                ...
                } else if($("input[name='ptype']").val() == 'ios'){
                ...
                }else if($("input[name='ptype']").val() == 'console'){
                    if (!match_console.test($("input[name='soft_name']").val())) {
                        notice("<%'上传文件应以【.exe】格式结尾'|L%>");
                        document.getElementById('light').style.display = 'none';
                        document.getElementById('fade').style.display = 'none';
                        return false;
                ...
                document.getElementById('light').style.display = 'none';
                document.getElementById('fade').style.display = 'none';
                notice1("<%'文件正在上传中请稍后'|L%>...",'?m=cms&a=index_console');
    ...
    if (pid == '' || pid == null || ptype != 'console') {
        ...
        if (con == "<%'取消'|L%>") {
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data) {
                        notice("<%'文件删除成功'|L%>", '?m=cms&a=index_console');
                    } else {
                        notice("<%'文件删除失败'|L%>");
                    }
                }
    ...
    if (pid == '' || pid == null || ptype != 'console') {
        ...
        if (con == "<%'取消'|L%>") {
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data) {
                        notice("<%'文件夹已清空'|L%>", '?m=cms&a=index_console');
                    } else {
                        notice("<%'文件夹清空失败'|L%>", '?m=cms&a=index_console');
                    }
                }
