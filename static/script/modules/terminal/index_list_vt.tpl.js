function edit_term(obj,id,imei,value,md_parent_ag,meid){
    ...
    $(obj).attr("onclick","edit_term(this,'"+id+"','"+imei+"','"+value+"','"+md_parent_ag+"','"+meid+"')");
    ...
    /*var md_meid=$("input[name=md_meid"+id+"]").val();*/
...
//        $("input[name=md_type"+id+"]").val(md_type);
    ...
    $(obj).html(shtml);
      ...
      $(obj).next().attr("onclick","cancel(this,'"+id+"','"+value+"','"+imei+"','"+md_type+"','"+md_parent_ag+"','"+meid+"');");
      return;
    }else{
        layer.closeAll('tips');
    }

    //MEID验证   VT版先不加 先注释状态 下面添加方法 未完待续
    /*var match=/^[0-9a-zA-Z]{14}$/;
    if(md_meid!=''){
        if(!match.test(md_meid)){
            $(obj).html(shtml);
            var index=layer.tips("<%'请输入14位数字字母'|L%>", $(obj).parent().prev().prev().prev().prev().prev().prev(),{
                tips:[1, '#A83A3A'],
            });
            $(obj).next().attr("onclick","cancel(this,'"+id+"','"+value+"','"+imei+"','"+md_type+"','"+md_parent_ag+"','"+meid+"');");
            return;
            ...
            success:function(msg){
                if(msg==1){
                    ...
                    $.ajax({
                        ...
                        dataType:"json",
                        data:{md_id:md_id,md_imei:md_imei,md_type:$("input[name=md_type_name"+id+"]").val(),md_parent_ag:md_parent_ag,md_serial_number:md_serial_number,md_time:md_time,do:"edit"},
                        success:function(msg){
                            if(msg.status==0){
                                layer.msg(msg.msg);
                            ...
                            //               console.log($(this).children().eq(3));
                                           ...
                                           var md_serial=$(this).children().eq(7).children().val();
                                           var id_md=that.attr("num");
                                           that.attr("onclick","edit_term(this,'"+id_md+"','"+imei+"','"+md_serial+"','"+md_parent_ag+"','"+meid+"')");
                                           that.next().attr("onclick","del_term(this,'"+md_id+"')");
                                       }
                                });
                                if(msg.info.md_serial_number==null){
                                    msg.info.md_serial_number="";
                                }
                                if(msg.info.e_name==null){
                                    msg.info.e_name="";
                                }
                                if(msg.info.md_binding_user==null){
                                    msg.info.md_binding_user="";
                                }
                                if(msg.info.md_time==null){
                                    msg.info.md_time="";
                                }
                                if(msg.info.ag_name==null){
                                    msg.info.ag_name="OMP";
                                }
                                var test = "IMEI:"+msg.info.md_imei+"<br /><%'终端型号'|L%>:"+msg.info.md_type+"<br /><%'序列号'|L%>:"+msg.info.md_serial_number+"<br /><%'所属代理'|L%>:"+msg.info.ag_name+"<br /><%'所属企业'|L%>:"+msg.info.e_name+"<br /><%'系统号码'|L%>:"+msg.info.md_binding_user+"<br /><%'入库时间'|L%>:"+msg.info.md_time;
...
//                                    $(obj).next().attr("onclick","cancel(this,'"+id+"');");
    ...
    $(obj).html(shtml);
    $(obj).next().html(chtml);
    $(obj).next().attr("onclick","cancel(this,'"+id+"','"+value+"','"+imei+"','"+md_type+"','"+md_parent_ag+"');");
}

function cancel(obj,id,value,imei,md_type,md_parent_ag){
    var ehtml = '<img class="edit" src="images/edit.png" onMouseOver="this.src=\'images/edit_pass.png\'" onMouseOut="this.src=\'images/edit.png\'"/>';
    var dhtml = '<img class="delete" src="images/delete.png" onMouseOver="this.src=\'images/delete_pass.png\'" onMouseOut="this.src=\'images/delete.png\'"/>';
    $("input.cb").removeAttr("disabled");
    $("a.link").removeClass("set_gray");
...
//               console.log($(this).children().eq(3));
             ...
             var md_serial=$(this).children().eq(7).children().val();
            var id_md=that.attr("num");
            that.attr("onclick","edit_term(this,'"+id_md+"','"+imei+"','"+md_serial+"','"+md_parent_ag+"')");
            that.next().attr("onclick","del_term(this,'"+md_id+"')");
           }
   });
     var md_type_name=$("input[name=md_type_name"+id+"]").val();
