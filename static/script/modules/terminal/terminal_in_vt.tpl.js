...
$(".export").click(function () {
    ...
    if (action === 'terminal_group') {
        url = "?m=terminal&a=terminal_export_vt";
    }
    $("#ifr").attr("src", url);
});
...
$("a.ajaxpost_t").click(function(){
    ...
    $("tr.number").each(function(){
        var that=$(this);
        serial_test(that.children().next().next().next().children().eq(0));
            var match=/^[0-9a-zA-Z]{15}$/;
            flag=true;
            if(!match.test(that.children().next().children().val())){
              layer.tips("<%'请输入15位数字或字母'|L%>", that.children().next().children().eq(0),{
                  tips:[1, '#A83A3A'],
              });
            ...
            $.ajax({
                ...
                data:{md_imei:that.children().next().children().val()},
                success:function(res){
                    if(res==1){
                        var index=layer.tips("<%'此IMEI已存在'|L%>", that.children().next().children().eq(0),{
                            tips:[1, '#A83A3A'],
                        });
                      //$("input.error:first").focus();
                      flag=false;
