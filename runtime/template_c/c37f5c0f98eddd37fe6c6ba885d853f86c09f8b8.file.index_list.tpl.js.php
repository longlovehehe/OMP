<?php /* Smarty version Smarty-3.1.11, created on 2016-05-13 10:08:33
         compiled from "..\static\script\modules\terminal\index_list.tpl.js" */ ?>
<?php /*%%SmartyHeaderCode:25946573537210373f7-95269316%%*/if(!defined('SMARTY_DIR')) exit('no direct access allowed');
$_valid = $_smarty_tpl->decodeProperties(array (
  'file_dependency' => 
  array (
    'c37f5c0f98eddd37fe6c6ba885d853f86c09f8b8' => 
    array (
      0 => '..\\static\\script\\modules\\terminal\\index_list.tpl.js',
      1 => 1448515086,
      2 => 'file',
    ),
  ),
  'nocache_hash' => '25946573537210373f7-95269316',
  'function' => 
  array (
  ),
  'has_nocache_code' => false,
  'version' => 'Smarty-3.1.11',
  'unifunc' => 'content_57353721102622_39268053',
),false); /*/%%SmartyHeaderCode%%*/?>
<?php if ($_valid && !is_callable('content_57353721102622_39268053')) {function content_57353721102622_39268053($_smarty_tpl) {?>function edit_term(obj,id,imei,value){
    ...
    $("tr").each(function(){
           if($(this).children().eq(10).text()!=""&&$(this).children().eq(10).text()=="<?php echo L('无');?>
"){
...
", $(obj).parent().prev().prev().prev().prev().prev().prev(),{
    ...
    if(res==false){
        var index=layer.tips("<?php echo L('只能输入数字和字母');?>
    ...
    if($(obj).children().attr('class')=="save"){
        $.ajax({
            ...
            success:function(msg){
                if(msg==1){
                    var index=layer.tips("<?php echo L('此IMEI已存在');?>
", $(obj).parent().prev().prev().prev().prev().prev().prev(),{
                                  tips:[1, '#A83A3A'],
                              });
                }else{
                        ...
                        success:function(msg){
                            if(msg.status==0){
                                ...
                                $("tr").each(function(){//保存成功后 恢复原有界面
                                    if($(this).children().eq(10).html()!=""&&$(this).children().eq(10).html()!="<?php echo L('无');?>
"){
                                        $(this).children().eq(11).children().addClass("set_gray");
                                    }else if($(this).children().eq(10).text()!=""&&$(this).children().eq(10).text()=="<?php echo L('无');?>
"){
...
function cancel(obj,id,value,imei,md_type){
    ...
    $("tr").each(function(){//保存成功后 恢复原有界面
           if($(this).children().eq(10).children().html()!=""&&$(this).children().eq(10).html()!="<?php echo L('无');?>
"){
               $(this).children().eq(11).children().addClass("set_gray");
           }else if($(this).children().eq(10).text()!=""&&$(this).children().eq(10).text()=="<?php echo L('无');?>
"){
...
function del_term(obj,id,md_imei){
    layer.confirm("<?php echo L('确定要删除该终端吗');?>
",{btn: ["<?php echo L('确定');?>
", "<?php echo L('取消');?>
"],title:"<?php echo L('删除');?>
"},function(){
            $.ajax({
                       ...
                       success:function(msg){
                           if(msg==2){
                               layer.msg("<?php echo L('终端被绑定不能删除');?>
");
                           }else if(msg==0){
