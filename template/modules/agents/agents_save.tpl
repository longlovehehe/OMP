...
<form id="form" class="base mrbt10" action="?modules=agents&action=save&ag_number={$data}">
    ...
    <div class="block">
        ...
        <input autocomplete="off"  value="{$info.ag_name}"  maxlength="32" chinese="true" name="ag_name" type="text" required="true" />
    </div>
    <div class="block">
        <label class="title">{"地址"|L}：</label>
        <input  autocomplete="off"  value="{$info.ag_addr}"  maxlength="32"  name="ag_addr" type="text" required="true" />
    </div>
    <div class="block">
    ...
    <div class="block">
        ...
        <select name="ag_area" class="autofix {if $res.do eq "edit"}autoeditselect{/if}" {if $res.do eq "edit"}disabled{/if} value='{$info.ag_area|escape:html}'  action="?m=area&a=option" selected="true" data='[{ "to": "e_mds_id","field": "d_area","view":"false" }]'>
    ...
    <div class="block">
        <label class="title">{"基本功能价格"|L}：</label>
         <input rmb="true" autocomplete="off" style="width:120px;" value="{$result.basic_price|default:0}"  maxlength="16"  name="basic_price" type="text" required="true" />
    </div>
    <div class="block">
        <label class="title">{"Console用户价格"|L}：</label>
        <input rmb="true" autocomplete="off" style="width:120px;" value="{$result.console_price|default:0}"  maxlength="16"  name="console_price" type="text" required="true" />
    </div>
    *}
