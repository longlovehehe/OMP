...
<div class="form mrbt20">
    ...
    <div class="block ">
        ...
        <span>{$data.e_location|mbsubstr:20}</span>
    </div>
    <div class="block">
        <label class="title">{"行业"|L}：</label>
        <span>{$data.e_industry|mbsubstr:20}</span>
    </div>
    <div class="block">
        <label class="title">{"联系人"|L}：</label>
       <span>{$data.e_contact_name}</span>&nbsp;
       <span>{$data.e_contact_surname|mbsubstr:20}</span>
    </div>
    <div class="block">
...
{*             <a href="?m=enterprise&a=move_mds&e_id={$data.e_id}" class="button " title='{"迁移{$smarty.session.ident}-Server的VCR服务器信息"|L}'>{"迁移所属"|L}</a>*}
            ...
            {*具有录制功能才可迁移VCR*}
            {if $data.e_id != '999999'}
{*            <a href="?m=enterprise&a=move_vcr&e_id={$data.e_id}" class="button" title="{'迁移企业的VCR服务器信息'|L}">{"迁移RS"|L}</a>*}
		
            {if $smarty.session.ag.ag_level neq '1'}
                <a href="javascript:;" id="move_enterprise" class="button" title="{'迁移所属'|L}">{"迁移所属"|L}</a>
            {/if}
            {/if}
            {*{/if}*}
