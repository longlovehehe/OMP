...
<form class="data">
    <table class="base full" >
        <tr class='head' id="user_list">
            <th width="50px"><input autocomplete="off"  type="checkbox" id="checkall" /></th>

            <th width="130px">{"姓名"|L}</th>
            <th width="130px">{"号码"|L}</th>
            <th class="rich none" width="100px">{"类型"|L}</th>
            <th class="rich" width="110px">{"增值功能"|L}</th>
            <th class="rich" width="120px">{"所属群组"|L}</th>
            <th class="rich" width="110px">{"部门"|L}</th>
            <th class="rich" width="120px">{"类型"|L}</th>
            ...
            <th class="rich" width="70px">{"详情"|L}</th>
            <th class="rich" width="70px">{"操作"|L}</th>
            <th class="rich" width="70px">{"历史记录"|L}</th>
        </tr>

        {foreach name=list item=item from=$list}
        <tr>
            <td><input onclick="getnum();" autocomplete="off"  type="checkbox" name="checkbox[]" value="{$item.u_number}" class="cb" /></td>
            <td title="{$item.u_name}"><em class="ellipsis" style="width: 265px">{$item.u_name|mbsubstr:5}</em></td>
            <td>{$item.u_number}</td>
            <td class="rich none">{$item.u_sub_type|modtype}</td>
            <td title='{$item.u_p_function|getEListBypjson}' class="rich">{"功能列表"|L}</td>
            <td class="rich">
            ...
            <td class="rich">{if substr($item.u_number,6,1) ==7}{4|modwordtype}{else}{$item.u_sub_type|modwordtype}{/if}{*{1|modwordstatus}{1|modwordprewarning}{1|modwordpreTraffic}*}</td>
            ...
            <td class="rich"><a  {if $item.u_sub_type eq 1}title="{'号码'|L}:【{$item.u_number}】<br />{'姓名'|L}:【{$item.u_name}】<br />{'类型'|L}:【{$item.u_sub_type|modtype}】<br />{'默认群组'|L}:【{$item.pg_name}】<br />{'部门'|L}:【{$item.ug_name}】<br />{'终端类型'|L}:【{$item.u_terminal_type}】<br />{'机型'|L}:【{$item.u_terminal_model}】<br />{*{'UDID'|L}:【{$item.u_udid}】<br />*}{'IMSI'|L}:【{$item.u_imsi}】<br />{'IMEI'|L}:【{$item.u_imei}】<br />{'ICCID'|L}:【{$item.u_iccid}】<br />{'MAC'|L}:【{$item.u_mac}】<br />{'购买日期'|L}:【{$item.u_purch_date}】<br />{'终端序列号'|L}:【{$item.u_terminal_number}】"{elseif $item.u_sub_type eq 2}title="{'号码'|L}:【{$item.u_number}】<br />{'姓名'|L}:【{$item.u_name}】<br />{'类型'|L}:【{$item.u_sub_type|modtype}】<br />{'默认群组'|L}:【{$item.pg_name}】<br />{'部门'|L}:【{$item.ug_name}】"{else}title="{'号码'|L}:【{$item.u_number}】<br />{'姓名'|L}:【{$item.u_name}】<br />{'类型'|L}:【{$item.u_sub_type|modtype}】<br />{'部门'|L}:【{$item.ug_name}】"{/if} class="link tips_title"><span class="icon hand"></span></a></td>
            ...
            <td class="rich"><a href="?m=enterprise&a=users_history&e_id={$data.e_id}&uh_u_number={$item.u_number}&do=edit&page={$page}" class="link view"></a></td>
        </tr>
        {/foreach}
    </table>
    {if $list!=NULL}
