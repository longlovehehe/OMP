...
<form id="form" class="base mrbt10" action="?m=enterprise&a=users_save_shell">
    ...
    <input autocomplete="off"  value='{$u_p_function_new}' name="u_p_function_new" type="hidden">
    ...
    <div class="block {if $data.do eq edit}none{/if}" >
    ...
    <div class="block">
       ...
       {if $data.do != 'edit'}
        <input autocomplete="off"   maxlength="32" value="" name="u_number"u_number="true" type="text" required="true" />
        {else}
        <span class="gradient_light">{$item.u_number}</span>
        <input autocomplete="off"  value="{$item.u_number}" name="u_number" type="hidden">
        {/if}
    </div>
    <div class="block">
    ...
    <div class="block">
        ...
        {*<input class="mobile-number" mobile1="true" type="text" style="height: 28px;width: 245px;border:1px solid #ccc;" name="u_mobile_phone" value="{$item.u_mobile_phone}" required="true"/>*}
    ...
    <div class="block sw user">
        ...
        <div><div class="title" style="width:220px;height: auto; border:1px solid #ccc; padding: 10px;clear:both;"><div id="product_select" class="autofix  autocheck"  value="{$item.u_p_function|escape:"html"}" action="?m=product&a=ip_option&e_id={$data.e_id}"></div><div style="color:#8B2929;padding-left: 5px;" class="change_function none"><div  style="padding-top:15px;"><div style="color:#000;padding-top: 80px;"><input type="checkbox" name="clean_p" value="0" />{"无增值功能"|L}</div><br />*{"次月生效"|L}:<div style="width: 180px; padding:10px 5px 10px 5px;float:left;border: 1px dashed #8B2929" id='show_change'></div><div><a href="javascript:void(0);" class="delete_func">×</a></div></div></div></div><div style="clear:both;margin-top: 10px;padding-left: 250px;">
        
        <input name="isused" type="checkbox" value="on"/><label>{"次月生效"|L}</label></div></div>
    </div>
    <div class="block">
    ...
    <div class="block radio" value="{$item.u_attr_type}">
        <div class="line {*{if $item.u_attr_type eq 0}{if $data.do eq edit}none{/if}{/if}*}">
            <label class="title">{"用户分类"|L}：</label>
            <label class="radiotext">
                <input autocomplete="off"  value="1" name="u_attr_type" type="radio"  {if $item.u_attr_type eq 0}{if $data.do eq edit}disabled="disabled"{/if}{/if}/>
                <span>{"测试"|L}</span>
            </label>
            <label class="radiotext">
                <input autocomplete="off"  value="0" name="u_attr_type" type="radio" checked="checked"  {if $item.u_attr_type eq 0}{if $data.do eq edit}disabled="disabled"{/if}{/if}/>
                <span>{"商用"|L}</span>
            </label>
            {if $item.u_attr_type eq 0}{if $data.do eq edit}<input type="hidden" name="u_attr_type" value="0"/>{/if}{/if}
        </div>
    </div>
    ...
    <div class="sw user block">
        ...
        <select name="u_gis_mode" class="autoedit" value="{$item.u_gis_mode|default:5}">
...
{*            <input autocomplete="off"   maxlength="32" value="{$item.u_terminal_type}" name="u_terminal_type" type="text"  />*}
            <select  name="u_terminal_type" value="{$item.u_terminal_type}" class="autoedit autofix" action="?m=terminal&a=option">
    ...
    <div class="auto_config {if $item.u_auto_config == 0}hide{/if} ">
        ...
        <div class="block user radio" value="{$item.u_bind_phone|default:0}">
            ...
            <div class="line">
                <label class="radiotext"><input autocomplete="off"  value="0" name="u_bind_phone" type="radio" checked="checked" />{"否"|L}</label>
            </div>
        </div>
        ...
        <div class="sw block user radio" value="{$item.u_checkup_grade|default: 1}">
    ...
    <div class="sw user">
        ...
        <div class="block">
            ...
            <input autocomplete="off" class="datepickers start"  maxlength="32" value="{$item.u_purch_date}" name="u_purch_date" type="text"  />
        </div>
        <div class="block">
            <label class="title">{"终端序列号"|L}：</label>
            <input autocomplete="off" chrnum="true"  maxlength="32" value="{$item.u_terminal_number}" name="u_terminal_number" type="text"  />
        </div>
