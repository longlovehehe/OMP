...
<form class="data">
    <table class="base full">
        ...
        <tr title="{'企业编号'|L}: 【{$item.e_id}】<br />{'企业名称'|L}: 【{$item.e_name}】<br />{'区域'|L}: 【{$item.e_area|mod_area_name}】<br />{'状态'|L}: 【{$item.e_status|modifierStatus}】 <br />{$smarty.session.ident}-Server: 【{$item.mds_d_name}】<br/>{$smarty.session.ident}-RS: 【{if $item.rs_d_name}{$item.rs_d_name}{else}{'无'|L}{/if}】<br/>{$smarty.session.ident}-SS: 【{if $item.ss_d_name}{$item.ss_d_name}{else}{'无'|L}{/if}】<br/>{'创建者'|L}:【{$item.e_create_name|getompman}】<br/>{'企业创建时间'|L}:【{$item.e_create_time}】">
            <td><input autocomplete="off"  type="checkbox" name="checkbox[]" value="{$item.e_id}" class="cb" {if $item.e_id eq 999999}disabled{/if}/></td>
            <td>{$item.e_id}</td>
