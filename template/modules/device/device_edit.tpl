...
<form id="form" class="base mrbt10" action="?modules=device&action=mds_save">
    ...
    <div class="block">
        ...
        <input autocomplete="off"   maxlength="32" name="d_ip1" value="{$data.d_ip1}" type="text" required="true" ip="true" />
    </div>
    <div class="block">
        <label class="title">{"外网地址"|L}：</label>
        <input autocomplete="off"   maxlength="32" name="d_ip2" value="{$data.d_ip2}" type="text" required="true" ip="true" />
        <input autocomplete="off"   maxlength="32" name="d_port1" type="hidden" required="true" value="2001" />
        <input autocomplete="off"   maxlength="32" name="d_port2" type="hidden" required="true" value="2001" />
    </div>
    <div class="block">
    ...
    <div class="block">
        ...
        {foreach name=list item=item key=key from=$aCluster}
            <option value="{$item.cluster_id}" {if $data.d_deployment_id eq $item.cluster_id}selected{/if}>{$item.cluster_name}</option>
        {/foreach}
        </select>
    </div>
    <div class="buttons mrtop40">
        <a goto="?m=device&a=server" form="form" class="ajaxpost button normal">{"保存"|L}</a>
