...
<form class="data">
    <table class="base full">
        ...
        <tr>
            ...
            <td class="rich"><a  title="{'ID'|L}: 【{$item.d_id}】<br />{'设备名称'|L}: 【{$item.d_name}】<br />{'部署方式'|L}: 【{$item.d_deployment_type|modDeploymentType}】<br />{'内网地址'|L}: 【{$item.d_ip1}】<br />{'外网地址'|L}: 【{$item.d_ip2}】<br />{'网络接入方式'|L}: 【{$item.d_network_type|modNetworkType}】<br />{'区域'|L}: 【{$item.d_area|mod_area_name}】<br />{'部署ID名称'|L}: 【{$item.cluster_name}】<br />{'用户总数'|L}: 【{$item.d_user}】<br />{'可用手机用户数'|L}: 【{$item.diff_phone|default:0} | {$item.d_phone_user|default:0}】<br />{'可用调度台用户数'|L}: 【{$item.diff_dispatch|default:0} | {$item.d_dispatch_user|default:0}】<br />{'可用GVS用户数'|L}: 【{$item.diff_gvs|default:0} | {$item.d_gvs_user|default:0}】<br />{'状态'|L}: 【{$item.d_status|modDeviceStatus}】<br />{'SIP端口'|L}: 【{$item.d_sip_port}】" class="link tips_title"><span class="icon hand"></span></a></td>
            <td>
                ...
                {/if}
                |<a href="javascript:void(0);" class="link" {if $item.d_area != '["#"]'}onclick="new_creat({$item.d_id});"{else}onclick="title_notice();"{/if}>{"区域"|L}</a>|<a href="?m=device&a=device_list&device_id={$item.d_id}&do=mds&d_ip2={$item.d_ip2}&d_deployment_id={$item.d_deployment_id}" class="link">{if $smarty.cookies.lang eq en_US}Info{else}详情{/if}</a>
            </td>
        </tr>
        {/foreach}
    </table>
