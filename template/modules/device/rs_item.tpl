...
<form class="data">
    <table class="base full">
        ...
        <tr>
            ...
            <td class="rich"><a  title="{'ID'|L}: 【{$item.d_id}】<br />{'设备名称'|L}: 【{$item.d_name}】<br />{'部署方式'|L}: 【{$item.d_deployment_type|modDeploymentType}】<br />{'内网地址'|L}: 【{$item.d_ip1}】<br />{'外网地址'|L}: 【{$item.d_ip2}】<br />{'网络接入方式'|L}: 【{$item.d_network_type|modNetworkType}】<br />{'部署ID名称'|L}: 【{$item.cluster_name}】<br />{'已用/总并发数'|L}: 【{$item.sum_recnum}|{$item.d_recnum}】<br />{'状态'|L}: 【{$item.d_status|modDeviceStatus}】<br />" class="link tips_title"><span class="icon hand"></span></a></td>
            <td>
                {if $item.status eq 'yes'}
                <a title="{'此设备下有企业在用不能编辑'|L}" class="link dis">{"编辑"|L}</a>
                {else}
                <a href="?m=device&a=rs_edit&d_id={$item.d_id}" class="link">{"编辑"|L}</a>
                {/if}
                <!-- |<a href="javascript:void(0);" class="link" {if $item.d_area != '["#"]'}onclick="new_creat({$item.d_id});"{else}onclick="title_notice();"{/if}>{"区域"|L}</a> -->|<a href="?m=device&a=rs_list&device_id={$item.d_id}&do=rs&d_name={$item.d_name}&d_ip2={$item.d_ip2}" class="link">{if $smarty.cookies.lang eq en_US}Info{else}详情{/if}</a>
            </td>
        </tr>
        {/foreach}
    </table>
