<div class="toolbar">
    ...
    <a href="?modules=device&action=vcr" class="button">VCR管理</a>
    <a href="?modules=device&action=vcrs" class="button active">VCR-S管理</a>
</div>
<h2 class="title">VCR-S列表</h2>

<div class="toptoolbar">
    <a href="?modules=device&action=device_add&d_type=vcrs" class="button orange">新增设备</a>
</div>
<div class="toolbar">
    <form action="?modules=device&action=vcr_item" id="form" method="post">
        ...
        <input autocomplete="off"  name="action" value="vcrs_item" type="hidden" />
        ...
        <div class="line">
            <label>设备IP地址：</label>
            <input autocomplete="off"  class="autosend" name="d_ip" type="text" />
        </div>
            ...
            $("#dialog-confirm").dialog({
                ...
                buttons: {
                    "删除": function () {
                        ...
                        notice("正在删除");
                        $.ajax({
                            url: "?modules=device&action=vcrs_del",
                            data: "list=" + checkd,
                            success: function (result) {
                                notice("成功删除 " + result + " 台设备");
                                setTimeout(function () {
                                    send();
                                }, 888);
                            }
