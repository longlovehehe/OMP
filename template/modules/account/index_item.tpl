<form class="data">
    <div class="form base mrbt10 toolbar" id="hide">
    <div class="block ">
    ...
    {*            <th width="80px">{"产品功能费"|L}</th>*}
        ...
        {*            <td title="产品功能费:{$item.sum}<br />基础功能费:{$item.basic_price}+{$item.console_price}">{$item.sum+$item.basic_price+$item.console_price|default:'暂无记录'}</td>*}
                    ...
                    <td width="80px">{$item.er_sum_money|string_format:"%.2f"}</td>
                    {if $smarty.request.ep_id eq "0"}
                        <td width="80px"><a href="?m=account&a=show_ep_info_emp&er_id={$item.er_id}&ep_id={$item.er_ag_path|mb_substr:4:12}&start={$res.start}" class="link edit">{"详情"|L}</a></td>
                    {else}
                        <td width="80px"><a href="?m=account&a=show_ep_info_amp&er_id={$item.er_id}&ep_id={$item.er_ag_path|mb_substr:4:12}&start={$res.start}" class="link edit">{"详情"|L}</a></td>
                    {/if}
                </tr>
                {/foreach}
            </table>
...
{*                <div style="float: left;padding-right: 250px;height: 24px;text-align: center;">{"注:Phone类型的用户的费用包含基础功能费+增值功能费"|L}</div>*}
                <div style="float: right;font-size: 24px;font-weight: bold; padding-right: 15px;">Total:<span>{$total|string_format:"%.2f"}</span></div>
    ...
    <div class="form mrbt10 toolbar" id="hide1">
    <div class="block ">
        <label class="title">{"其他费用"|L}：</label>
        <input oninput='get_price();' oncopy="return false;" onpaste="return false;" oncut="return false;" type="number" min="0"  maxlength="64" autocomplete="off"  name="other_price" class="autosend calculation" style="width:180px;height: 24px;border-bottom: 5px solid #cccc;text-align: center;"/>
    </div>
    <div class="line ">
    ...
    <div class="line ">
        ...
        <span><input type="text" oncopy="return false;" onpaste="return false;" oncut="return false;" maxlength="64" autocomplete="off" name="pre_total" class="autosend calculation" disabled="true" value="{$total|string_format:"%.2f"}" style="width:180px;height: 24px;border-bottom: 5px solid #cccc;text-align: center;"/></span>
    </div>
    <div class="line ">
        <label class="title">{"税率"|L}：</label>
        <span><input type="number" oninput="get_pte();" min="0" oncopy="return false;" onpaste="return false;" oncut="return false;" maxlength="8" autocomplete="off" name="pte" class="autosend calculation" value="" style="width:180px;height: 24px;border-bottom: 5px solid #cccc;text-align: center;"/>%</span>
    </div>
        <br />
        <br />
    <div class="line" style="float: right;font-size: 20px;font-weight: bold;">
        <label>{"税后合计"|L}：</label>
        <span><input type="text"  name="total" disabled="true" autocomplete="off" class="autosend" value="{$total|string_format:"%.2f"}" style="width:120px;height: 24px;border-bottom: 5px solid #cccc;text-align: center;"/></span>
    </div>
    </div>
</form>
  <script>
