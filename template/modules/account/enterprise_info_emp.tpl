<div class="toptoolbar ">
    ...
    <a href="?m=account&a=index&start={$smarty.request.start}" class="button orange goto_back">{"返回"|L}</a>
</div>
   ...
   <div class="block ">
    ...
    <table class="base full">
        <tr class='head' >
            ...
            <th width="90px">{"用户类型"|L}</th>
            <th width="150px">{"增值功能"|L}</th>
            <th width="90px">{"开户日期"|L}</th>
        ...
        {foreach name=list key=key item=item from=$list}
            {if $smarty.foreach.list.iteration eq $item.count}
                <tr>
                  ...
                  <td width="45px">{$smarty.foreach.list.iteration}</td>
                  <td width="80px">{$item.ur_name|mbsubstr:6:"..."}</td>
                  <td width="80px">{$item.ur_number}</td>
                  <td width="90px">{"{$item.ur_sub_type}"|L}</td>
                  <td width="150px">{$item.ur_p_function}</td>
                  <td width="90px">{$item.ur_create_time|default:'0'}</td>
                  <td width="75px">{$item.ur_sum_money|string_format:"%.2f"}</td>
              </tr>
              <tr>
                  <td width="45px"></td>
                  <td width="80px"></td>
...
{*                <div style="float: left;padding-right: 250px;height: 24px;text-align: center;">{"注:Phone类型的用户的费用包含基础功能费+增值功能费"|L}</div>*}
    ...
    <div class="line ">
        ...
        <span><input type="number" oninput="get_price();" min="0" oncopy="return false;" onpaste="return false;" oncut="return false;" maxlength="64" autocomplete="off" name="other_price" class="autosend calculation" style="width:180px;height: 24px;border-bottom: 5px solid #cccc;text-align: center;"/></span>
    </div>
    <div class="line ">
    ...
    <div class="line ">
        ...
        <span><input type="text" oncopy="return false;" onpaste="return false;" oncut="return false;" autocomplete="off" name="pre_total" readonly class="autosend" value="{$total_price|string_format:"%.2f"}" style="width:180px;height: 24px;border-bottom: 5px solid #cccc;text-align: center;"/></span>
    </div>
    <div class="line ">
    ...
    <div class="line" style="float: right;font-size: 20px;font-weight: bold;">
        <label>{"税后合计"|L}：</label>
        <span><input type="text" name="total" disabled="true" class="autosend" value="{$total_price|string_format:"%.2f"}" style="width:120px;height: 24px;border-bottom: 5px solid #cccc;text-align: center;"/></span>
    </div>
    </div>
</form>
    <!--输出台-->
