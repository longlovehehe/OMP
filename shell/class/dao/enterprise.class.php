...
class enterprise extends db {
	...
	public function getDeviceList() {
		...
		} else if ($this->data["do"] == "rs") {
			$pagesql = "SELECT count(e_vcr_id) as total FROM \"T_Enterprise\" WHERE \"e_vcr_id\"=" . $this->data["device_id"];
		} else {
			$pagesql = "SELECT count(e_ss_id) as total FROM \"T_Enterprise\" WHERE \"e_ss_id\"=" . $this->data["device_id"];
		}
		...
		switch ($this->data["do"]) {
			...
			case "rs":
				...
				break;
			case "ss":
				$sql = "SELECT e_id,e_name,e_vcr_audiorec,e_vcr_videorec,e_vcr_space FROM \"T_Enterprise\" WHERE e_ss_id=:e_ss_id";
				break;
		}
		...
		switch ($this->data["do"]) {
			...
			case "rs":
				...
				break;
			case "ss":
				$sth->bindValue(':e_ss_id', $this->data["device_id"], PDO::PARAM_INT);
				break;
		}
...
SQL;
	...
	function saveMDS() {
		...
		if ($edit) {
			...
			$sth->bindValue(':e_create_name', $_SESSION['own']['om_id'], PDO::PARAM_STR);
			$sth->bindValue(':e_agents_id', '0');
			$sth->bindValue(':e_ag_path', '|0|');
		}
		try {
			...
			if ($edit) {
				$log = DL("修改") . " " . $log;
				$msg["msg"] = L("修改") . " " . L('企业 %s 失败，企业ID：%s 事件ID：%s' . $ex);
	...
	function getWhere($order = false) {
		if ($_SESSION['own']['om_id'] != "admin") {
			$where = " WHERE 1=1 AND e_id!=999999 AND e_status !=6";
		} else {
			$where = " WHERE 1=1 AND e_status !=6";
		}
		...
		if ($this->data["e_move_path"] != "" && $this->data["e_create_name"] != "") {
			if ($this->data["e_create_name"] == "0") {
				$ag_path = '|0||' . $this->data["e_move_path"] . '|';
				$where .= "AND e_ag_path = '{$ag_path}'";
			} else {
				$where .= "AND e_ag_path LIKE E'%" . addslashes($this->data["e_create_name"]) . "%'";
			}
		}
	...
	function getWhere_record($order = false) {
		if ($this->data['ag_number'] == "0") {
			$where = " WHERE 1=1 AND er_ag_path ='|{$this->data['ag_number']}|'";
		} else {
			$where = " WHERE 1=1 AND er_ag_path like '%{$this->data['ag_number']}%'";
		}
		...
		$area = new area($_REQUEST);
		$where .= $area->getAcl('er_area', $_SESSION["own"]['om_area']);

		if ($order) {
			$where .= ' ORDER BY er_name';
		}
		return $where;
	}
	...
	function getWhere_ag($order = false) {
		if ($this->data['ag_number'] == "0") {
			$where = " WHERE 1=1 AND e_ag_path ='|{$this->data['ag_number']}|'";
		} else {
			$where = " WHERE 1=1 AND e_ag_path like '%|{$this->data['ag_number']}|%'";
		}
		if (trim((int) $this->data["e_id"]) > 0) {
			$where .= "AND TEXT(e_id) LIKE E'%" . (int) $this->data["e_id"] . "%'";
		}
	 ...
	 * 编辑代理商时用户许可查找
	 */
	function getWhereag($order = false) {
		$where = " WHERE 1=1 AND e_create_name='{$this->data['ag_number']}'";
		if (trim((int) $this->data["e_id"]) > 0) {
			$where .= "AND TEXT(e_id) LIKE E'%" . (int) $this->data["e_id"] . "%'";
		}
		if ($this->data["e_name"] != "") {
...
//            if($this->redis==0){
		return $result;
//            }else{
//                if($this->redis->exists(md5($sql))){
//                    return unserialize($this->redis->get("key"));
//                }else{
//                    $this->redis->set(md5($sql),  serialize($result));
//                }
//            }
	}
...
ECHO;
	...
	public function delList() {
		$list = implode(", ", $this->data["checkbox"]);
// 安全删除
		$sql = "SELECT e_id FROM \"T_Enterprise\" WHERE e_id IN($list) AND \"T_Enterprise\".e_status != 1 AND \"T_Enterprise\".e_id != 999999";
...
// 合并device VCR
	...
	public function moveVCR() {
		...
		$aEn = $this->getByid();
		if (!$aEn['e_vcr_id']) {
			$sql = 'UPDATE "T_Enterprise" SET e_vcr_id = :e_vcr_id,e_rs_rec = :e_rs_rec,e_has_vcr = :e_has_vcr WHERE e_id = :e_id';
		} else {
		...
		if (!$aEn['e_vcr_id']) {
			...
			$sth->bindValue(':e_rs_rec', $e_rs_rec, PDO::PARAM_INT);
			$sth->bindValue(':e_has_vcr', '1', PDO::PARAM_INT);
		}
...
ECHO;
	 ...
	 * [获得某设备下的所选企业的用户许可数量]
	 * @Author   longfei.wang
	 * @DateTime 2015-12-17T10:12:49+0800
	 * @param    [string]                   $e_id_list [所选企业ID字符串 之间用，隔开]
	 * @return   [array]                              [返回一个数组]
	 */
	public function get_allep_sure($e_id_list) {
		$sql = <<<echo
    	SELECT 
    		SUM(e_mds_phone) AS phone_sure,
    		SUM(e_mds_dispatch) AS dispatch_sure,
    		SUM(e_mds_gvs) AS gvs_sure,
    		SUM(e_rs_rec) AS rs_sure
	FROM "T_Enterprise" 
	WHERE 
	e_id IN ($e_id_list)
echo;
	 ...
	 * @return   [array]                              [返回一个数组]
	 */
	public function get_allep_alldevice($e_id_list) {
		$sql = <<<echo
    	SELECT 
    		e_mds_id,
    		e_vcr_id,
...
echo;
	...
	public function move_ep($e_id, $oldinfo) {

		$sql = <<<ECHO
    	UPDATE "T_Enterprise" SET e_mds_id=:e_mds_id,e_vcr_id=:e_vcr_id,e_ss_id=:e_ss_id,e_status=:e_status WHERE e_id=:e_id
ECHO;
		$sth = $this->pdo->prepare($sql);
		$sth->bindValue(':e_id', $e_id, PDO::PARAM_INT);
		$sth->bindValue(':e_mds_id', $this->data['e_mds_id'], PDO::PARAM_INT);
		$sth->bindValue(':e_vcr_id', $this->data['e_vcr_id'] == "" ? NULL : $this->data['e_vcr_id'], PDO::PARAM_INT);
		$sth->bindValue(':e_ss_id', $this->data['e_ss_id'], PDO::PARAM_INT);
		$sth->bindValue(':e_status', $this->data['e_status'], PDO::PARAM_INT);
		// $sth->bindValue(':e_area', json_encode(trim($this->data['e_area'],"\"")),PDO::PARAM_STR);
		try {
			$sth->execute();
			$newinfo = $this->data;
			unset($newinfo['e_status']);
			$this->make_history_args($oldinfo, $newinfo);
		} catch (Exception $exc) {
		...
		if (!empty($aKey)) {
			foreach ($aKey as $key => $value) {
				if ($key == 'e_vcr_id' && $value == '') {
					...
					array_push($arr, $key . " = '" . json_encode($value) . "'");
				} else {
	...
	public function get_agents() {
		$e_info = $this->getByid();
		$agents = new agents();
		if ($e_info['e_agents_id'] == '0' || $e_info['e_agents_id'] == '') {
			$ag_list = $agents->getAllag();
			...
			if ($ag_info['ag_level'] == '1') {
				//omp平台下 企业所属代理商为二级代理时，则不能进行企业迁移操作
				$result = 'nothing';
			} else {
				//当企业所属代理为一级代理，在omp平台上只能迁移到omp下
				$result['list'][0]['ag_number'] = '0';
				$result['list'][0]['ag_name'] = 'OMP';
				//$result['omp'] = true;
			}
		}
		return $result;
	...
	public function checkAgents($ag_list) {
		...
		foreach ($ag_list as $key => $value) {
			...
			$gvs = 0; //企业所分配GVS数
			//下级代理的手机、调度台、调度台用户的和
			$ag = new agents(array('aggents_number' => $value['ag_number'], 'ag_level' => '0'));
			...
			$ep = new enterprise(array('ag_number' => $value['ag_number'], 'e_create_name' => $value['ag_number'], 'e_move_path' => $value['ag_number']));
			$info = $ep->getList();
	...
	public function make_history_args($old, $new) {
		...
		if ($old['e_location'] == '' && $new['e_location'] == '') {
			unset($old['e_location']);
			unset($new['e_location']);
		}
...
ECHO;
			...
			$sth->bindValue(":eh_do_username", $_SESSION['own']['om_id']);
			$sth->execute();
...
ECHO;
	...
	public function get_eq_concurrent($list_ep) {
		if ($list_ep == "") {
			$list_ep = NULL;
		}
		$sql = <<<ECHO
                    SELECT SUM(e_rs_rec) FROM "T_Enterprise" WHERE e_id IN($list_ep);
ECHO;
		$sth = $this->pdo->query($sql);
		$res = $sth->fetch();
		return $res;
	...
	public function get_device_concurrent($e_vcr_id) {
		if ($e_vcr_id == "") {
			$e_vcr_id = NULL;
		}
		$sql = <<<ECHO
                    SELECT SUM(e_rs_rec) FROM "T_Enterprise" WHERE e_vcr_id=$e_vcr_id;
ECHO;
		$sth = $this->pdo->query($sql);
		$res = $sth->fetch();
		return $res;
