...
class pttmember extends db {
	...
	public function del() {
		...
		$userinfo = $users->getuser($this->data['pm_number']);
		if ($userinfo["u_default_pg"] == $this->data['pg_number']) {
			$users->delForPtm();
		}
	...
	function delGVSptt() {
		...
		$sth->bindValue(':pm_number', $this->data['u_number']);
		$sth->execute();
		$log = DL('删除用户【%s】所属群组');
		$log = sprintf($log
			, $this->data['u_number']
    ...
    public function save() {
            if ($this->data["do"] == "edit" && !$this->checkUser($this->data['pm_number'], $this->data['pm_pgnumber'])) {
                ...
                if($this->data['pm_level']==""){
                    ...
                    //$log = DL("修改企业用户") . ":【%s】(%s)" . DL("群组信息成功") . "<br /> " . DL("企 业 群 组") . " :【%s】(%s)<br />" . DL("群 组 级 别") . " :【%s】<br />" . DL("默 认 群 组") . " :【%s】<br />" . DL("被叫挂断权限") . ":【%s】";
                ...
                $log = sprintf($log
                    ...
                    $groups = new groups(array('pg_number'=>  $this->data['pm_pgnumber']));
                    $pg_name=$groups->getbyid();
...
//                        , $this->data["pg_name"]
                ...
                $pm_hangup = $this->data["pm_hangup"]==null?0:$this->data["pm_hangup"];
                if($this->data['pm_level']==""){
...
//var_dump($this->data);die;
		...
		if ($this->data["do"] == "edit") {
			...
			$log = DL("修改企业用户") . ":【%s】(%s)" . DL("群组信息成功") . "<br />" . DL("用 户 级 别") . " :【%s】";
			$log = sprintf($log
				, $user_name
				, $this->data['pm_number']
				, $this->data['pm_level']
			...
			$log = DL('企业群组') . ':【%s】(%s)<br />' . DL('新增用户') . ':【%s】(%s) <br />' . DL('用户级别') . ':【%s】 <br />';
			$log = sprintf($log
				, $pg_name['pg_name']
				, $pg_name['pg_number']
				, $user_name
