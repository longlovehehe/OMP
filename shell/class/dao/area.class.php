...
class area extends db {
	...
	public function update() {
		...
		try
		{
			...
			if ($ex->getCode() == 23505) {
				$log = DL('修改区域名称失败。原因') . '  ' . DL('名称重复');
				$this->log($log, 4, 1);
				return $this->msg(L('修改区域名称失败。原因') . '  ' . L('名称重复'), -1);
			}
			$log = DL('修改区域名称失败。原因') . '：' . $ex->getMessage();
			$this->log($log, 4, 2);
			return $this->msg(L('修改区域名称失败。原因') . '：' . $ex->getMessage(), -1);
		}
...
SQL;
	...
	function getWhere($order = false) {
		$where = " WHERE 1=1 ";
		$where .= $this->getAcl('am_id', $_SESSION['own']['om_area']);
