...
class cms extends db {
	...
	public function upload_soft($info) {
		...
		if($this->data['ptype'] == 'console')
		{
			if ($info['soft_name']['size'] >= '62914560' || $info['soft_name']['size'] == 0) {
			return false;
			}
		}
		else
...
//          $p_time=data("Y-m-d");
    ...
    public function upload_soft_keeper($info) {
...
//          $p_time=data("Y-m-d");
            ...
            if (!$edit) {
                ...
                if($this->data['path']==""){
                    $sql = 'UPDATE "public"."T_Packet" SET p_dir = :p_dir,p_version = :p_version,p_time = :p_time WHERE p_type = :p_type';
                    $sth = $this->pdo->prepare($sql);
                }else{
                    $sql = 'UPDATE "public"."T_Packet" SET p_dir = :p_dir,p_file = :p_file,p_version = :p_version,p_time = :p_time,p_content = :p_content WHERE p_type = :p_type';
                    $sth = $this->pdo->prepare($sql);
                    $sth->bindValue(':p_content', $p_content, PDO::PARAM_LOB);                 
                }
                    
            }
            $sth->bindValue(':p_dir', "XXX");
            $sth->bindValue(':p_version', addslashes($this->data["ptt_version"]));
            $sth->bindValue(':p_time', date('Y-m-d H:i:s',time()), PDO::PARAM_INT);
            $sth->bindValue(':p_type', $this->data["p_type"]);
            if($soft_name!=""){
                  $sth->bindValue(':p_file', addslashes($soft_name));
            }
	...
	public function getconsoleList($aKey = array())
	{
		$sql = 'SELECT p_id,p_type,p_version,p_file,p_dir FROM "T_Packet"';
		$sql = $sql . $this->getWhere($aKey);
	...
	public function getWhere($aKey = array())
	{
		if(!empty($aKey))
		{
			$where = " WHERE 1=1 ";
...
//
	...
	public function get_keeper($ptype) {
		$sql = 'SELECT p_id,p_dir,p_file,p_version,p_time,p_type FROM "T_Packet" WHERE p_type = :p_type';
		$sth = $this->pdo->prepare($sql);
		$sth->bindValue(':p_type', $ptype);
		$data = $sth->execute();
		$data = $sth->fetch(PDO::FETCH_ASSOC);
