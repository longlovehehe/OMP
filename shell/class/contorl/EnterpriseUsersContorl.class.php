...
class EnterpriseUsersContorl extends contorl {
    ...
    private function importUserCheck() {
            ...
            for ($row = 2; $row <= $highestRow; $row++) {
                    ...
                    $u_auto_run = trim($objWorksheet->getCellByColumnAndRow(17, $row)->getValue());
                    $u_checkup_grade = trim($objWorksheet->getCellByColumnAndRow(18, $row)->getValue());
                    $u_encrypt = trim($objWorksheet->getCellByColumnAndRow(19, $row)->getValue());
                    $u_audio_mode = trim($objWorksheet->getCellByColumnAndRow(20, $row)->getValue());
                    $u_gis_mode = trim($objWorksheet->getCellByColumnAndRow(21, $row)->getValue());
                    //$u_sex = trim($objWorksheet->getCellByColumnAndRow(22, $row)->getValue());
                    //$u_position = trim($objWorksheet->getCellByColumnAndRow(23, $row)->getValue());
                    $u_terminal_type = trim($objWorksheet->getCellByColumnAndRow(24, $row)->getValue());
                    //$u_terminal_model = trim($objWorksheet->getCellByColumnAndRow(25, $row)->getValue());
                    //$u_zm = trim($objWorksheet->getCellByColumnAndRow(26, $row)->getValue());
                    $u_purch_date = trim($objWorksheet->getCellByColumnAndRow(27, $row)->getValue());
                    $u_terminal_number = trim($objWorksheet->getCellByColumnAndRow(28, $row)->getValue());

                    $u_numbe = trim($objWorksheet->getCellByColumnAndRow(0, $row)->getValue());
                    if (Cof::re('/^\d{11})$/', $u_numbe, 64) === 0) {
                            $error[] = "第 $row 行 用户号码 $u_numbe 不是纯数字";
                    }
                    ...
                    if ($u_auto_config === '1') {
                            ...
                            // GPS定位上报方式
                            if (Cof::re('/[012345\s]/', $u_gis_mode, 20) === 0) {
                                    if ($u_gis_mode == '') {
                    ...
                    //var_dump(Cof::re('/^\s*$|^[0-9a-zA-Z]$/i',$u_terminal_type , 32));die;
                    if (Cof::re('/^\s*$|^[0-9a-zA-Z]$/i', $u_terminal_type, 32) === 0) {
	...
	private function importUser() {
		...
		for ($row = 2; $row <= $highestRow; $row++) {
			...
			$userData['u_auto_run'] = trim($objWorksheet->getCellByColumnAndRow(17, $row)->getValue());
			if ($userData['u_auto_run'] == '') {
				$userData['u_auto_run'] = 0;
			}

			$userData['u_checkup_grade'] = trim($objWorksheet->getCellByColumnAndRow(18, $row)->getValue());
			if ($userData['u_checkup_grade'] == '') {
				$userData['u_checkup_grade'] = 1;
			}

			$userData['u_encrypt'] = trim($objWorksheet->getCellByColumnAndRow(19, $row)->getValue());
			if ($userData['u_encrypt'] == '') {
				$userData['u_encrypt'] = 0;
			}

			$userData['u_audio_mode'] = trim($objWorksheet->getCellByColumnAndRow(20, $row)->getValue());

			if ($userData['u_audio_mode'] == '') {
				$userData['u_audio_mode'] = 0;
			}

			$userData['u_gis_mode'] = trim($objWorksheet->getCellByColumnAndRow(21, $row)->getValue());
			if ($userData['u_gis_mode'] == '') {
				$userData['u_gis_mode'] = 0;
			}

			//$userData['u_sex'] = trim($objWorksheet->getCellByColumnAndRow(22, $row)->getValue());
			//$userData['u_position'] = trim($objWorksheet->getCellByColumnAndRow(23, $row)->getValue());
			$userData['u_terminal_type'] = trim($objWorksheet->getCellByColumnAndRow(24, $row)->getValue());
			$userData['u_terminal_model'] = trim($objWorksheet->getCellByColumnAndRow(25, $row)->getValue());
			//$userData['u_zm'] = trim($objWorksheet->getCellByColumnAndRow(26, $row)->getValue());
	...
	function users_batch_ug() {
		...
		foreach ($_REQUEST['checkbox'] as $value) {
			...
			$this->user->batchUser_ug();
                        if($res['u_ug_id']!=$_REQUEST['u_ug_id']){
                            $flag=true;
                        }
		}
                if($flag==true){
...
/**
    ...
    function users_batch() {
            ...
            foreach ($_REQUEST['checkbox'] as $value) {
                    ...
                    //$data['u_p_function'] = json_encode($_REQUEST['u_p_function']);
                    ...
                    $data['u_attr_type'] = $_REQUEST['u_attr_type'];
