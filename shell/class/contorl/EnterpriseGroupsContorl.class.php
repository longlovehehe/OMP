...
class EnterpriseGroupsContorl extends contorl {
	...
	private function importPTCheck() {
		...
		for ($row = 2; $row <= $highestRow; $row++) {
			...
			if ($tmpName !== '') {
				$wz = $tmpName;
				if (Cof::re('/^[\x{4e00}-\x{9fa5}\da-zA-Z()（） ._]+$/u', $tmpName, 64)===0) {
	...
	private function importPT() {
		...
		for ($row = 2; $row <= $highestRow; $row++) {
			...
			if ($tmpName !== '') {
				...
				if (!Cof::re('/^[\x{4e00}-\x{9fa5}\da-zA-Z()（） ._]+$/u', $tmpName, 64)) {
					$error[] = "第 $row 行，$tmpName 包含 A-z0-9#.-_（）()以外的字符";
				}
			}
			$tmpuser = array();
...
//选中群组是否为紧急对讲组
                            ...
                            if ($info["pg_level"] == 0) {
                                    ...
                                    //var_dump($_REQUEST['checkbox']);die;
	...
	public function getcustalluser() {
		...
		$list = $user->getalluser();
		foreach ($list as $val) {
			$pg_list[$val['u_number']] = $this->custpgmember->getcustPgname($val['u_number'],$val['u_default_pg']);
		}
		$this->smarty->assign('data', $_REQUEST);
		$this->smarty->assign('list', $list);
