        ...
        function index_item() {
                ...
                if ($_REQUEST['do'] == 'console') {
                        ...
                        foreach ($list as $key => $value) {
                            if($value['e_id']!="999999"){
                                $arr_list[$key]['e_id']=$value['e_id'];
                                $arr_list[$key]['e_name']=$value['e_name'];
                            }
                        }
...
//                $gvs_num = $array['diff_gvs'] + $array['sum_gvs'];
                ...
                if($_REQUEST['did']=="edit"){
                    ...
                    $phone_num = $array['diff_phone']+$epdata['e_mds_phone'] ;
                    $dispatch_num = $array['diff_dispatch']+$epdata["e_mds_dispatch"] ;
                    $gvs_num = $array['diff_gvs']+$epdata["e_mds_gvs"];
                }else{
                ...
                try
                {
                        if ($_REQUEST["e_id"] == "") {
                                ...
                                if ($_REQUEST["e_has_vcr"] == "") {
                                    if($data['e_id'] == '999999')
                                    {
                                        $this->tools->send("SpecialExEdit", $msg["e_id"]);
                                    }
                                    else
                                    {
                                        if($aE['e_status'] != '5')
...
//                        $this->tools->send("ExCreate", $data["e_id"]);
    ...
    function edit() {
            ...
            if($data['e_vcr_id'])
            {
                $aKey['device_id'] = $data['e_vcr_id'];
                $device = new device($aKey);
                $aRs = $device->getRec();
                $this->smarty->assign('aRs', $aRs);
            }
...
$e_info = $enterprise->get_history_list();
    ...
    */
    ...
    function move_vcr() {
            ...
            $data = $enterprise->getByid();
            $this->smarty->assign('data', $data);
            $this->render('modules/enterprise/move_rs.tpl', L('迁移GQT-RS'));
    }
