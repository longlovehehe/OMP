...
function modssid($str) {
	...
	// $data['d_ip1'] += $epdata['d_ip1'];
	$data['name'] = $data['d_name'] . '【' . $data['d_ip2'] . '】' . L('已用空间') . '：' . floor($data['space']/1024) . '|' . L('总空间') . '：' . floor($data['d_space']/1024) ;
	return '[' . json_encode($data) . ']';
}
...
function get_termtype($str,$id){
    $tem=new terminal();
    $res=$tem->getList();
    $option="<select name='md_type".$id."' value='$str'>";
    $selected="";
    foreach ($res as $key => $value) {
        if($str==$value["tt_type"]){
            $selected="selected";
        }else{
            $selected="";
        }
        $option.="<option value='{$value["tt_type"]}' $selected>{$value['tt_type']}</option>";
    }
    $option.="</select>";
    //$select="<select name='tl_model".$id."' value='".$str."'>".$option."</select>";
    return $option;
}

function getterm_name($str){
    $tem=new terminal(array('tt_id'=>$str));
    $res=$tem->getById_type();
    return $res['tt_type'];
}

function get_bind($str){
    $user=new users();
    $res=$user->getimei($str);
    if(count($res)==0){
        return 0;
    }else{
        return 1;
...
function get_used($str){
         $term=new terminal();
         $res=$term->get_type_used($str);
         return $res;
    }
    
function get_bindnum($str){
    $user=new users();
    $res=$user->getunum($str);
    return $res;
}
/**
 * 用户状态
 ...
 * @return string
 */
function get_isbind($type,$imei){
        switch ($type) {
                case '0':
                        return "<img src='./images/unbind.png' />";
                        break;

                case '1':
                        $term=new terminal(array("md_imei"=>$imei));
                        $res=$term->getselect_list();
                        switch ($res['md_status']){
                                case '0':
                                        return L("<img src='./images/disable.png' />");
                                        break;
                                case '1':
                                        return L("<img src='./images/enable.png' />");
                                        break;
                                case NULL:
                                        return L("<img src='./images/disable.png' />");
                                        break;
 ...
 * @return int
 */
function get_isstat($imei){
	$term=new terminal(array("md_imei"=>$imei));
	$res=$term->getselect_list();
	if($res['md_status']==null){
		$res['md_status']=0;
	}
	return $res['md_status'];
}
 ...
 * @return string 处理中|正常|异常
 */
function modDeploymentType($str) {
	switch ($str) {
		case 0:
			return L("本地部署");
		case 1:
